...
PyPlugins CIMIScriptableView::m_plugins;
...
void
CIMIScriptableView::attachIC(CIMIContext* pIC)
{
...
unsigned
CIMIScriptableView::clearIC(void)
{
    if (!m_pIC->isEmpty()) {
...
void
CIMIScriptableView::updateWindows(unsigned mask)
{
    if (!m_pWinHandler)
        return;
...
bool
CIMIScriptableView::onKeyEvent(const CKeyEvent& key)
{
...
int
CIMIScriptableView::onCandidatePageRequest(int pgno, bool relative)
{
    unsigned changeMasks = 0;
...
int
CIMIScriptableView::onCandidateSelectRequest(int index)
{
    unsigned changeMasks = 0;
...
void
CIMIScriptableView::getPreeditString(IPreeditString& ps)
{
    ps.clear();
...
void
CIMIScriptableView::getCandidateList(ICandidateList& cl, int start, int size)
{
    cl.clear();
    cl.reserve(size);
...
void
CIMIScriptableView::_erase (bool backward, unsigned &changeMasks)
{
    if (backward) {
...
void
CIMIScriptableView::_getCandidates ()
{
    m_candiPageFirst = 0;
...
void
CIMIScriptableView::_commitChar (TWCHAR ch)
{
    TWCHAR wa[2] = {ch, 0};
...
void
CIMIScriptableView::_commitString (const wstring& wstr)
{
    m_pWinHandler->commit(wstr.c_str());
}

void
CIMIScriptableView::_doCommit (bool bConvert)
{
    wstring bs;
...
unsigned
CIMIScriptableView::_moveLeft (unsigned& mask, bool searchAgain)
{
    if (m_cursorFrIdx == 0)
...
unsigned
CIMIScriptableView::_moveLeftSyllable (unsigned& mask, bool searchAgain)
{
    if (m_cursorFrIdx == 0)
        return _moveEnd (mask);
...
unsigned
CIMIScriptableView::_moveHome (unsigned& mask, bool searchAgain)
{
    if (m_cursorFrIdx == 0)
        return 0;
...
unsigned
CIMIScriptableView::_moveRight (unsigned& mask)
{
    if (m_cursorFrIdx < m_pIC->getLastFrIdx ()) {
        mask |= PREEDIT_MASK;
        ++ m_cursorFrIdx;
...
unsigned
CIMIScriptableView::_moveRightSyllable (unsigned& mask)
{
    if (m_cursorFrIdx < m_pIC->getLastFrIdx ()) {
        mask |= PREEDIT_MASK;
...
unsigned
CIMIScriptableView::_moveEnd (unsigned& mask)
{
    if (m_cursorFrIdx < m_pIC->getLastFrIdx ()) {
        mask |= PREEDIT_MASK;
...
void
CIMIScriptableView::_makeSelection (int candiIdx, unsigned& mask)
{
...
void
CIMIScriptableView::_deleteCandidate (int candiIdx, unsigned& mask)
{
    candiIdx += m_candiPageFirst;
    if (!m_tailSentence.empty ()) --candiIdx;
...
wstring
CIMIScriptableView::_plugin_trans(const wstring& str)
{
	if (m_plugins) {
		return m_plugins.trans(str);
	} else {
		return str;
	}
}
